...
volatile unsigned int one_second_flag=0; // Used for calculating the frequency
...
void SetupTimer2 (void)
{
	...
	__builtin_disable_interrupts();
	PR2 = (SYSCLK/DEF_FREQ)-1;
	TMR2 = 0; // start the timer at zero
...
int main(void)
{
	...
	while(1)
	{
		...
		TMR2 = 0; // Reset the timer count
		T2CONbits.ON = 1; // Start the timer
		one_second_flag = 0; // Reset the 1 second flag counter
		while(!one_second_flag); // while one second has not passed do nothing and wait
		...
		*/
		while (PORTB&(1<<3));
		StartPlayback(0x006cc1, 0x012ddb);
		if(TMR2 >= 44060 || TMR2 <= 45000)
		{
			StartPlayback(0x00, 0x3ff);
		} else {
			if(TMR2 >= 45000 || 45060)
			{
				StartPlayback(0x3d4, 0x3ff);
			}
		}
